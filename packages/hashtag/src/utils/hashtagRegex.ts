// Initial Source: https://github.com/draft-js-plugins/draft-js-plugins/blob/2973e9fd3f8536794ad49334c098822e7b7ddc67/draft-js-hashtag-plugin/src/utils/hashtagRegex.js

// Some minimizers convert string escapes into their literal values, which leads to intermittent Unicode normalization bugs and increases the gzipped download size. Use RegEx literals as opposed to string literals to prevent that.
...
const regexes: { [key: string]: RegExp } = {
  hashSigns: /[#＃]/,
  hashtagAlpha: new RegExp('[' + unicodeLettersAndMarks + ']'),
  hashtagAlphaNumeric: new RegExp(
    '[' + unicodeLettersAndMarks + unicodeNumbers + hashtagSpecialChars + ']'
  ),
  hashtagBoundary: new RegExp(
    '(?:^|$|[^&' +
...
const regexSupplant = (regex: RegExp): RegExp => {
  return new RegExp(
    regex.source.replace(/#\{(\w+)\}/g, (_match, name): string => {
      const newRegex = regexes[name]
      return newRegex ? newRegex.source : ''
    }),
    regex.flags
  )
}
